...
Super::Super(int x, int y,int type) : Enemy("play/super.png", x, y, 50, 50, 20, 1000,type) 
{
    timeTicks=0;
    // for (int i = 1; i <= 8; i++) {
...
void Super::CreateBullet() {
    
}

void Super::Draw() const
{
...
void Super::OnExplode(float deltatime)
{
    timeTicks+=deltatime;
    if(timeTicks>=timeSpan) timeTicks-=timeSpan;
